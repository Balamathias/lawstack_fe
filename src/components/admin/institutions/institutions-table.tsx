...
import { useDeleteInstitution } from '@/services/client/institutions'

interface InstitutionsTableProps {
  institutions: PaginatedStackResponse<Institution[]>
}

const InstitutionsTable = ({ institutions }: InstitutionsTableProps) => {
  ...
  const totalPages = Math.ceil(institutions.count / 10) // Assuming 10 items per page
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b">
              ...
              <th className="text-left py-3 px-4 font-medium text-sm hidden lg:table-cell">Location</th>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between">
          <p className="text-sm text-muted-foreground">
            ...
            <span className="font-medium">{Math.min(currentPage * 10, institutions.count)}</span> of{' '}
            <span className="font-medium">{institutions.count}</span> institutions
          </p>
